...
def find_nonexistent_species(sp_list, data_collection):
 	...
 	for sp in sp_list:	
 		...
 		if sp_info is None: #does not have any data 		
 			...
 			if (type(sp_info['images']) == types.DictType) and sp_info['eol_id'] != -1: #need to get image data
...
def insert_species_info(species_info, data_collection): 	
 	if type(species_info) == types.DictType:
 		result = data_collection.insert_one(species_info)
 		ids = result.inserted_id
 	elif type(species_info) == types.ListType: 
 		#print species_info	
...
def images_exists(species):
 	...
 	if species_info is None: 
 		...
 		if type(images_obj) == types.DictType: #need to get image data and update record
 			return {"image_info_exists": False, "species": species}
 		elif type(images_obj) == types.ListType:
