...
def get_tree_image():
    ...
    #print ("leftoffset: %s topoffset: %s") %(leftoffset, topoffset)
    #print "treeid: " + str(treeid) + " newick: " + newick
...
def get_action():
    ...
    if treeid and nodeid:
        ...
        for aindex, aname, aorder, html_generator in h.get_avail_actions(nodeid):
            node = h.get_tree_node(nodeid)
            if html_generator:
               html += html_generator(aindex, treeid, nodeid, node)
            else:
               html += """<li style="font-size:12pt"><a  onClick="run_action('%s', '%s', '%s', '%s');" >%s</a></li>""" %(treeid, nodeid, '', aindex, aname)
        html += "</ul>"
...
def start_server(host="phylo.cs.nmsu.edu", port=8990):
