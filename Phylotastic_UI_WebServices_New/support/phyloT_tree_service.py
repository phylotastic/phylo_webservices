...
def get_tree_itol(file_id, ncbiIdDict):
	...
	# parameter description: instead of uploading a tree, iTOL can automatically generate one from a file containing a list of NCBI tax IDs. NCBI taxonomy will be pruned based on your IDs and a Newick tree generated. Input file location
 	...
 	if not upload_status:
 		return {"message": "ItoL upload error:" + itol_uploader.comm.upload_output, "status_code": 500, "newick": ""}
 	else:    
 		...
 		if upload_output.find("SUCCESS") == -1: # SUCCESS: 174561252042902514884188540
 			return {"message": "No tree found using phyloT" , "newick": "", "status_code": 200}
...
def get_ncbi_ids(taxaList):
 	...
 	for taxon_name in taxaList:
 		...
 		if taxon_result['status_code'] == 200 and len(taxon_result['taxon_ids']) > 0:
...
#---------------------------------------------
def service_controller(taxaList):
 	...
 	if len(NCBI_dict) != 0:
		...
		get_tree_response = {'newick': "", 'status_code': 200, 'message': "No match found for input taxa"}

 	final_result = {}
 	
 	final_result['newick'] = get_tree_response['newick']			
    ...
    #service result creation time
 	...
 	source_urls = ["http://phylot.biobyte.de/"]	
...
#-----------------------------------------------
 	...
 	#input_list = ["Lufengpithecus lufengensis","Lufengpithecus keiyuanensis", "Lufengpithecus hudienensis"]
