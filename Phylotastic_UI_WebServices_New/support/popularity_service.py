...
def get_ott_id(taxon):
    opentree_url = "https://api.opentreeoflife.org/v3/tnrs/match_names"
    ...
    if response.status_code == requests.codes.ok:
        if len(result_data_json['results'])!= 0:    
            ...
            for match in matches:
               taxon_match = {}
               taxon_match['unique_name'] = match['taxon']['unique_name']
               taxon_match['ott_id'] = match['taxon']['ott_id']
...
def get_popular_species(taxon="biota", num_taxa=20):
    ...
    if taxon == "biota":
       ...
       if match_taxon_result['status_code'] != 200:
          ...
          for indx, match in enumerate(taxon_matches):
             ott_id = match['ott_id']
             #print str(ott_id)
             onezoom_result = onezoom_single_taxon_api(ott_id, num_taxa)
             if onezoom_result is None:
                continue
             elif onezoom_result['status_code'] == 200:
             ...
             elif onezoom_result['status_code'] != 200 and indx != len(taxon_matches)-1:
                continue
             else:
