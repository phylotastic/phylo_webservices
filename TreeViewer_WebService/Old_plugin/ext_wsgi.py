...
from wsgi import extapp # Required to use the webplugin
...
def example_app(environ, start_response, queries):
    ...
    start_response('202 OK', [('content-type', 'text/plain'),('Access-Control-Allow-Origin','*'),('Access-Control-Allow-Credentials','true')])
...
def clear_bg(node):
    node.add_feature("bgcolor","#FFFFFF")
...
def tree_renderer(tree, treeid, application):
    ...
    html_features = """
      ...
      <img src="/TreeViewer/close.png" onclick='$(this).closest("#tree_features_box").hide();'>
      </div>
 ...
 #       html_features += '<td><INPUT size=7 type="text"></td> <td><input size=7 type="text"></td> <td><input size=7 type="text"></td>  <td><input size=1 type="text"></td><br>'
    ...
    # <img width=16 height=16 src="/TreeViewer/icon_tools.png" alt="Select Tree features">
    # </a></li>"""

    download_button = """
     <li><a href="/TreeViewer/tmp/%s.png" target="_blank">
    ...
    #      show_box(event, box); '>
    #  <img width=16 height=16 src="/TreeViewer/icon_search.png" alt="Search in tree">
    #  </a></li>"""

    #clean_search_button = """
    #  <li><a href="#" onclick='run_action("%s", "", %s, "clean::clean");'>
    #  <img width=16 height=16 src="/TreeViewer/icon_cancel_search.png" alt="Clear search results">
    #  </a></li>""" %\
    #    (treeid, 0)
    
    buttons = '<div id="ete_tree_buttons">' + download_button + '</div>'

    #buttons = '<div id="ete_tree_buttons">' +\
    #    features_button + download_button +\
    #    '</div>'
    ...
    search_form = """
     ...
     <img src="/TreeViewer/close.png" onclick='$(this).closest("#search_in_tree_box").hide();'>
     </div>
    ...
    tree_panel_html = '<div id="tree_panel">' + html_features + buttons + '</div>'
...
if __name__ == '__main__':
   ...
   application = extapp.WebTreeApplication()
   ...
   # directory. Note that the referred directory must be writable by the webserver.

   #application.CONFIG["temp_dir"] = "/var/www/html/TreeViewer/tmp/"
   #application.CONFIG["temp_url"] = "/TreeViewer/tmp/" # Relative to web site Document Root
   ...
   # We extend the minimum WebTreeApplication with our own WSGI application
...
# ==============================================================================
   ...
   application.register_action("Clear background", "node", clear_bg, None, None)
